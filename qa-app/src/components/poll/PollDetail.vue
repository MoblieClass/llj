...
const votingStatus = ref('idle'); // 'idle', 'loading', 'success', 'error'
  ...
  if (props.currentUser) {
    try {
      const response = await fetch(
        `http://localhost:8080/api/polls/${localPoll.value.id}/has-voted`,
        {
          headers: {
      ...
      if (response.ok) {
        hasVoted.value = await response.json();
      }
    } catch (error) {
...
<template>
  <div class="poll-detail">
    ...
    <!-- 投票区域 -->
<div v-if="poll.isActive && !hasVoted && currentUser" class="voting-area">
  <h3>请选择一个选项进行投票</h3>
  ...
  <div v-for="option in poll.options" :key="option.id" class="option-vote">
    <button 
  ...
  <!-- 未登录提示 -->
  <div v-if="poll.isActive && !currentUser" class="login-prompt">
    <p>请<button @click="$emit('need-login')" class="inline-btn">登录</button>后参与投票</p>
  </div>

  <!-- 已投票提示 -->
  <div v-if="poll.isActive && hasVoted" class="voted-prompt">
    <p>您已对本投票进行了投票</p>
  </div>

  <!-- 已结束提示 -->
  <div v-if="!poll.isActive" class="ended-prompt">
    <p>本投票已结束</p>
  </div>
...
.login-prompt, .voted-prompt, .ended-prompt {
  text-align: center;
  margin-bottom: 20px;
  padding: 15px;
  background-color: #f0f0f0;
